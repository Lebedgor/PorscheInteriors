...
export const addInterior = createAsyncThunk("interior/addInterior", async (params, { rejectWithValue }) => {
  try {
    const { data } = await axios.post("interior", params)

    return data
  } catch (error) {
    return rejectWithValue(getErrorPayload(error, 'Не вдалося створити інтерʼєр'))
  }
})

export const updateInterior = createAsyncThunk("interior/updateInterior", async (params, { rejectWithValue }) => {
  try {
    const { data } = await axios.patch(`interior`, params)

    return data
  } catch (error) {
    return rejectWithValue(getErrorPayload(error, 'Не вдалося оновити інтерʼєр'))
  }
})
...
const carInteriorSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    .addCase(getInteriors.fulfilled, (state, action) => {
      ...
      state.model = action.payload.model?.[0] || null;
    })
