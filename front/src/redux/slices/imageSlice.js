...
export const updateImage = createAsyncThunk("image/updateImage", async (params) => {
  const { data } = await axios.patch('image', params)
...
export const showMoreImages = createAsyncThunk("image/showMoreImages", async ({ id, page }) => {
  ...
  const  { data }  = await axios.get(`interior_images/${id}/?page=${normalizedPage}`)
