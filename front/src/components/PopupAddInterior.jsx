...
import { formatImageBeforeUpload, getCachedImagePath, getThumbnailImageStyle } from '../imageSettings'
...
const PopupAddInterior = (props) => {
  ...
  const normalizedModelId = String(props.params.model_id || '').trim()
  ...
  const pushInterior = async() => {
    ...
    if (!normalizedModelId) {
      alert('Не вдалося визначити модель для інтерʼєру')
      return
    }
    ...
    const payload = {
      ...
      model_id: Number(normalizedModelId)
    }

    if (props.params.interior_id) {
      payload.interior_id = props.params.interior_id
    }
    ...
    let savedInterior = false

    if(!props.params.interior_id){
      savedInterior = await dispatch(addInterior(payload))
    } else {
      savedInterior = await dispatch(updateInterior(payload))
    }

    if(savedInterior.meta.requestStatus === 'fulfilled'){
      props.changeFlag()
      props.popupToggle()
      return
    }

    alert(getValidationMessage(savedInterior.payload, 'Не вдалося зберегти інтерʼєр'))
  }
    ...
    <div className={props.popupState ? "popup-container active" : "popup-container"}>
      <div className="popup-inner-container">
        <div className="popup-title-container">
          <div className="popup-title">Добавление варианта интерьера</div>
          ...
          <div className="model-form">
            ...
            { props.params.image ? 
              <div className="upload-image-container">
                <img src={getImageUrl(getCachedImagePath(props.params.image, 'thumbnail', imageSettings))} style={getThumbnailImageStyle(imageSettings)} alt="" />
            ...
            <button onClick={pushInterior} className="auth-btn">Сохранить</button>
          </div>
        </div>
...
export default PopupAddInterior
